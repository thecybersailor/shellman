...
function normalizeAppendFrameData(text: string) {
  // Some repaint deltas are effectively full snapshots wrapped as append.
  // Trim one trailing newline to keep cursor row aligned with prompt row.
  if (text.startsWith(ANSI_REPAINT_PREFIX) && text.endsWith("\n")) {
